...
type CreateUseCase interface {
	Execute(ctx context.Context, model *domain.DashboardUser) (*domain.DashboardUser, []response.ErrorField, error)
}
...
func (u *createUseCase) Execute(ctx context.Context, model *domain.DashboardUser) (*domain.DashboardUser, []response.ErrorField, error) {
	existingModel, err := u.repo.ExistsByUniqueFields(ctx, model.Email.String())
	if err != nil {
		return nil, nil, err
	}
	if existingModel {
